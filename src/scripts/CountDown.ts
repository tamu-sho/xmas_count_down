export default class CountDown {
  ...
  childElmList: {date: HTMLElement, hours: HTMLElement, min: HTMLElement, sec: HTMLElement}
  timer: number
  ...
  coundDown() {
    const timeLimit = this.getTimeLimit(this.date, new Date())
    this.displayTime({
      'date': timeLimit.date,
  ...
  getTimeLimit(futureDate: Date, nowDate: Date): {
    date: number, hours: number, min: number, sec: number
  } {
  ...
  displayTime(val: { date: number, hours: number, min: number, sec: number }) {
    this.childElmList.date.innerHTML = `${( '00' + val.date).slice(-1 * Math.max(val.date.toString().length, 2) )}`
  ...
  setDateElm(parent: HTMLElement): {
    date: HTMLElement, hours: HTMLElement, min: HTMLElement, sec: HTMLElement
  } {
